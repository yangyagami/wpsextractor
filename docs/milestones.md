...
| M2 | v0.2.0 | .wps 文字提取：ZIP Reader + XML Parser + wps_text + wps_binary | ✅ 已完成 |
| M3 | v0.3.0 | .et 表格提取 | ✅ 已完成 |
...
- [x] 解析 `xl/sharedStrings.xml`，构建共享字符串表 (SST)
- [x] 解析 `xl/worksheets/sheet*.xml`，提取每个单元格的文本
- [x] 支持 `t="s"`（共享字符串索引）
- [x] 支持 `t="inlineStr"`（内联字符串）
- [x] 支持直接值（数值等）
- [x] 多工作表之间以 `"\n--- Sheet N ---\n"` 分隔
- [x] 集成到 `wpsext_extract_file()`
- [x] 通过自动化测试验证
...
| 日期 | 内容 | 备注 |
|------|------|------|
| 2026-05-11 | 完成 et_table.c，零警告编译通过 | 创建最小 .et 测试文件验证通过 |
